...
module EvilInsultGeneratorUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = EvilInsultGeneratorResponse.new({})
    ...
    if fetch_err
      ...
      response = EvilInsultGeneratorResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = EvilInsultGeneratorResponse.new(fetched)
    else
