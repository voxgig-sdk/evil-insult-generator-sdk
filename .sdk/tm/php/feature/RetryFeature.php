...
// EvilInsultGenerator SDK retry feature
...
class EvilInsultGeneratorRetryFeature extends EvilInsultGeneratorBaseFeature
{
    ...
    public function init(EvilInsultGeneratorContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (EvilInsultGeneratorContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(EvilInsultGeneratorContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(EvilInsultGeneratorContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
