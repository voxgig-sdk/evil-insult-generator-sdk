...
// EvilInsultGenerator SDK feature test
//
...
class FtCtrl extends EvilInsultGeneratorControl
{
...
class FtClient
{
    ...
    private EvilInsultGeneratorUtility $utility;

    public function __construct(EvilInsultGeneratorUtility $utility, array $options)
    {
    ...
    public function get_utility(): EvilInsultGeneratorUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (EvilInsultGeneratorContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public EvilInsultGeneratorContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (EvilInsultGeneratorContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = EvilInsultGeneratorConfig::make_feature($name);
    ...
    private static function buildUrl(EvilInsultGeneratorSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof EvilInsultGeneratorError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new EvilInsultGeneratorResponse($fetched);
        }
    ...
    private function fail(EvilInsultGeneratorContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (EvilInsultGeneratorContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
