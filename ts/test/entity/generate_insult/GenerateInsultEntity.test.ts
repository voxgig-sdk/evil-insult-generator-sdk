...
import { EvilInsultGeneratorSDK, BaseFeature, stdutil } from '../../..'
...
describe('GenerateInsultEntity', async () => {
  ...
  test('instance', async () => {
    ...
    const ent = testsdk.GenerateInsult()
    assert(null != ent)
  })
  ...
  test('basic', async (t) => {
    ...
    const live = 'TRUE' === process.env.EVIL_INSULT_GENERATOR_TEST_LIVE
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'generate_insult.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set EVIL_INSULT_GENERATOR_TEST_GENERATE_INSULT_ENTID JSON to run live')
      return
    }
    ...
    let generate_insult_ref01_data = Object.values(setup.data.existing.generate_insult)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/generate_insult/GenerateInsultTestData.json')
  ...
  let client = EvilInsultGeneratorSDK.test(options, extra)
  ...
  const idmapEnvVal = process.env['EVIL_INSULT_GENERATOR_TEST_GENERATE_INSULT_ENTID']
  ...
  const live = 'TRUE' === env.EVIL_INSULT_GENERATOR_TEST_LIVE
  ...
  const setup = {
    ...
    struct,
    data: entityData,
    explain: 'TRUE' === env.EVIL_INSULT_GENERATOR_TEST_EXPLAIN,
    live,
