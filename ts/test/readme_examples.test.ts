...
//     (\`new EvilInsultGeneratorSDK(...)\` / \`EvilInsultGeneratorSDK.test(...)\` become
//     \`EvilInsultGeneratorSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'EvilInsultGeneratorSDK'
...
const TEST_SEED = {"entity":{"generate_insult":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new EvilInsultGeneratorSDK(...)\` and any
// \`EvilInsultGeneratorSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+EvilInsultGeneratorSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/EvilInsultGeneratorSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
