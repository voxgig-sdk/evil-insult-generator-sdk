...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("EVILINSULTGENERATOR_TEST_LIVE") == "TRUE" ||
		os.Getenv("EVILINSULTGENERATOR_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("EVILINSULTGENERATOR_TEST_EXPLAIN"); explain != "" {
