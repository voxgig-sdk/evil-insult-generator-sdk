# evil-insult-generator-mcp

[MCP](https://modelcontextprotocol.io) server exposing the EvilInsultGenerator SDK as
two agent tools — `evil-insult-generator_list` and `evil-insult-generator_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/evil-insult-generator-mcp)
make build

# 2. Provide credentials via the environment
export EVIL_INSULT_GENERATOR_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user evil-insult-generator \
  -- /absolute/path/to/evil-insult-generator-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./evil-insult-generator-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// evil-insult-generator_list: first page of records
{ "entity": "generate_insult" }
{ "entity": "generate_insult", "query": { } }

// evil-insult-generator_load: one record by id
{ "entity": "generate_insult", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/evil-insult-generator-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user evil-insult-generator \
     -- "$PWD"/dist/*/evil-insult-generator-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `evil-insult-generator_list` and `evil-insult-generator_load` tools now appear
   in new sessions. Ask the agent to *"list generate_insult using evil-insult-generator"*
   and it calls `evil-insult-generator_list` with `{"entity":"generate_insult"}`.
...
```sh
export EVIL_INSULT_GENERATOR_APIKEY=sk_live_xxx            # API key
export EVIL_INSULT_GENERATOR_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `evil-insult-generator_list` tool
...
### Call the `evil-insult-generator_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `evil-insult-generator_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `evil-insult-generator_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `EVIL_INSULT_GENERATOR_APIKEY` | API key sent with every request. |
| `EVIL_INSULT_GENERATOR_BASE` | Optional override of the API base URL. |
...
```sh
./evil-insult-generator-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"evil-insult-generator_load","arguments":{"entity":"generate_insult","query":{"id":1}}}}'
```
