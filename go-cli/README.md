# evil-insult-generator-cli

boru-driven command-line client **and** interactive REPL for the EvilInsultGenerator
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/evil-insult-generator-cli)
make build

# 2. See usage (words, entities, env vars)
./evil-insult-generator-cli --help

# 3. Provide credentials once, via the environment
export EVIL_INSULT_GENERATOR_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./evil-insult-generator-cli load 1 generate_insult            # {id:1} shorthand
./evil-insult-generator-cli load '{id:1}' generate_insult       # explicit match map

# 5. Override the API base URL for a single call
EVIL_INSULT_GENERATOR_BASE=https://api.example.com ./evil-insult-generator-cli load 1 generate_insult

# 6. No arguments -> interactive REPL
./evil-insult-generator-cli
evil-insult-generator> load 1 generate_insult
evil-insult-generator> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/evil-insult-generator-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/evil-insult-generator-cli load 1 generate_insult
   ```
...
```sh
./evil-insult-generator-cli load 1 generate_insult          # scalar shorthand for {id:1}
./evil-insult-generator-cli load '{id:1}' generate_insult     # explicit match map
```
...
./evil-insult-generator-cli load 1 generate_insult
```
...
Run with no arguments to open a REPL (prompt `evil-insult-generator>`). Each line is
evaluated as its own boru expression:

```text
$ ./evil-insult-generator-cli
evil-insult-generator> load 1 generate_insult
evil-insult-generator> /help
evil-insult-generator> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `generate_insult`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/evil-insult-generator-cli`. |
...
(`./evil-insult-generator-cli <expr>`) and interactively (the REPL), and expressions compose the
