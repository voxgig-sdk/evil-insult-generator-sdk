// Command EvilInsultGenerator-cli is a CLI + REPL for the EvilInsultGenerator SDK,
...
const prompt = "evil-insult-generator"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "generate_insult"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      EVIL_INSULT_GENERATOR_APIKEY (api key), EVIL_INSULT_GENERATOR_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: EVIL_INSULT_GENERATOR_APIKEY carries the API key and
	// EVIL_INSULT_GENERATOR_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("EVIL_INSULT_GENERATOR_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("EVIL_INSULT_GENERATOR_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewEvilInsultGeneratorSDK(opts)
