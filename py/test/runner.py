# EvilInsultGenerator SDK test runner
...
class EvilInsultGeneratorTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            EvilInsultGeneratorTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = EvilInsultGeneratorTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = EvilInsultGeneratorTestRunner.getenv("EVIL_INSULT_GENERATOR_TEST_LIVE")
        override = EvilInsultGeneratorTestRunner.getenv("EVIL_INSULT_GENERATOR_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = EvilInsultGeneratorTestRunner.getenv(key)
        ...
        explain = EvilInsultGeneratorTestRunner.getenv("EVIL_INSULT_GENERATOR_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if EvilInsultGeneratorTestRunner._test_control is not None:
            return EvilInsultGeneratorTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                EvilInsultGeneratorTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            EvilInsultGeneratorTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = EvilInsultGeneratorTestRunner.load_test_control()
...
def load_env_local():
    EvilInsultGeneratorTestRunner.load_env_local()


def env_override(m):
    return EvilInsultGeneratorTestRunner.env_override(m)


def entity_data(v):
    return EvilInsultGeneratorTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return EvilInsultGeneratorTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return EvilInsultGeneratorTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return EvilInsultGeneratorTestRunner.load_test_control()


def live_delay_ms():
    return EvilInsultGeneratorTestRunner.live_delay_ms()
