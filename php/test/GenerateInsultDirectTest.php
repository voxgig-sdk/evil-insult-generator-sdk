...
class GenerateInsultDirectTest extends TestCase
{
    public function test_direct_load_generate_insult(): void
    {
        $setup = generate_insult_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-generate_insult", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
...
function generate_insult_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new EvilInsultGeneratorSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
