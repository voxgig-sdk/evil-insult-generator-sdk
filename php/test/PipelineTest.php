...
// EvilInsultGenerator SDK pipeline test
//
...
{
    private static function utility(): EvilInsultGeneratorUtility
    {
        return new EvilInsultGeneratorUtility();
    }
    ...
    private static function ctx(array $over = []): EvilInsultGeneratorContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new EvilInsultGeneratorContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new EvilInsultGeneratorOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof EvilInsultGeneratorError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new EvilInsultGeneratorOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = EvilInsultGeneratorMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new EvilInsultGeneratorOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = EvilInsultGeneratorMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new EvilInsultGeneratorOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = EvilInsultGeneratorMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new EvilInsultGeneratorSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = EvilInsultGeneratorMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        ...
        [, $err] = EvilInsultGeneratorMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new EvilInsultGeneratorSpec([]),
            'response' => null,
            'result' => new EvilInsultGeneratorResult([]),
        ]);
        [, $err] = EvilInsultGeneratorMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new EvilInsultGeneratorSpec(['step' => 's']),
            'response' => new EvilInsultGeneratorResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new EvilInsultGeneratorResult([]),
        ]);
        [, $err] = EvilInsultGeneratorMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new EvilInsultGeneratorSpec(['step' => 's']),
            'response' => new EvilInsultGeneratorResponse(self::resp(200, ['v' => 1])),
            'result' => new EvilInsultGeneratorResult([]),
        ]);
        [, $err] = EvilInsultGeneratorMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new EvilInsultGeneratorSpec(['step' => 's']),
            'response' => new EvilInsultGeneratorResponse(self::resp(200, ['v' => 2])),
            'result' => new EvilInsultGeneratorResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        EvilInsultGeneratorMakeResponse::call($ctx);
    ...
    {
        $preset = new EvilInsultGeneratorResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new EvilInsultGeneratorSpec([]),
            'response' => new EvilInsultGeneratorResponse([]),
            'result' => new EvilInsultGeneratorResult([]),
        ...
        [$got, $err] = EvilInsultGeneratorMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new EvilInsultGeneratorResult([])]);
        [, $err] = EvilInsultGeneratorMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new EvilInsultGeneratorSpec([]), 'result' => null]);
        [, $err] = EvilInsultGeneratorMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new EvilInsultGeneratorSpec(['step' => 's']),
            'result' => new EvilInsultGeneratorResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new EvilInsultGeneratorOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = EvilInsultGeneratorMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new EvilInsultGeneratorSpec(['step' => 's']),
            'result' => new EvilInsultGeneratorResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new EvilInsultGeneratorOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = EvilInsultGeneratorMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new EvilInsultGeneratorResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new EvilInsultGeneratorSpec([]),
            'result' => new EvilInsultGeneratorResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = EvilInsultGeneratorMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = EvilInsultGeneratorMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new EvilInsultGeneratorError('boom', 'boom');
        $utility->fetcher = function (EvilInsultGeneratorContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new EvilInsultGeneratorSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = EvilInsultGeneratorMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (EvilInsultGeneratorContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = EvilInsultGeneratorMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(EvilInsultGeneratorResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        EvilInsultGeneratorMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (EvilInsultGeneratorContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new EvilInsultGeneratorSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = EvilInsultGeneratorMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new EvilInsultGeneratorResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new EvilInsultGeneratorSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = EvilInsultGeneratorMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = EvilInsultGeneratorMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = EvilInsultGeneratorMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new EvilInsultGeneratorResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, EvilInsultGeneratorDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new EvilInsultGeneratorResult(['ok' => false])]);
        $this->expectException(EvilInsultGeneratorError::class);
        EvilInsultGeneratorDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new EvilInsultGeneratorResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', EvilInsultGeneratorMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        EvilInsultGeneratorMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new EvilInsultGeneratorBaseFeature();
        $b = new EvilInsultGeneratorBaseFeature();
        EvilInsultGeneratorFeatureAdd::call($ctx, $a);
        EvilInsultGeneratorFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): EvilInsultGeneratorBaseFeature
    {
        $f = new EvilInsultGeneratorBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        EvilInsultGeneratorFeatureAdd::call($ctx, self::named_feature('a'));
        EvilInsultGeneratorFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        EvilInsultGeneratorFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        EvilInsultGeneratorFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        EvilInsultGeneratorFeatureAdd::call($ctx, $replace);
        ...
        EvilInsultGeneratorFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): EvilInsultGeneratorContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new EvilInsultGeneratorSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = EvilInsultGeneratorPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        EvilInsultGeneratorPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new EvilInsultGeneratorResponse(['headers' => null]),
            'result' => new EvilInsultGeneratorResult([]),
        ]);
        EvilInsultGeneratorResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        EvilInsultGeneratorResultBody::call($ctx);
