...
// EvilInsultGenerator SDK GenerateInsult entity
...
class GenerateInsultEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new GenerateInsultEntity($this->_client, $opts);
    }

    /**
     * @param GenerateInsult|array $args GenerateInsult data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = EvilInsultGeneratorHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GenerateInsult|array The current GenerateInsult data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GenerateInsult fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = EvilInsultGeneratorHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GenerateInsult fields).
     */
    ...
    /**
     * Load a single GenerateInsult.
     *
     * @param GenerateInsultLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed GenerateInsultLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GenerateInsult|array The loaded GenerateInsult as an assoc-array at the
     *   SDK boundary; throws EvilInsultGeneratorError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = EvilInsultGeneratorHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
