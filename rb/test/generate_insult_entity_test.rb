...
class GenerateInsultEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = generate_insult_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "generate_insult." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set EVIL_INSULT_GENERATOR_TEST_GENERATE_INSULT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    generate_insult_ref01_data = nil
    if generate_insult_ref01_data_raw.length > 0
...
def generate_insult_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "generate_insult", "GenerateInsultTestData.json")
  ...
  entid_env_raw = ENV["EVIL_INSULT_GENERATOR_TEST_GENERATE_INSULT_ENTID"]
  ...
  if env["EVIL_INSULT_GENERATOR_TEST_LIVE"] == "TRUE"
    merged_opts = Vs.merge([
      {
      },
      extra || {},
    ])
    client = EvilInsultGeneratorSDK.new(Helpers.to_map(merged_opts))
  end
